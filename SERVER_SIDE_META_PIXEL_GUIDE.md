# Server-Side Meta Pixel Implementation Guide

This guide explains how to implement server-side Meta Pixel tracking in your WordPress backend to complement the client-side tracking in your React application.

## Overview

Server-side Meta Pixel tracking offers several advantages over client-side tracking:

1. **Improved Reliability**: Works even when browser tracking is blocked by ad blockers
2. **Better Conversion Tracking**: More accurate attribution since tracking happens server-side
3. **Enhanced Privacy Compliance**: Can be configured to hash sensitive data
4. **Cross-Device Tracking**: Better attribution across user sessions

## WordPress Integration

### Step 1: Add Server-Side Tracking Code to WordPress

Add the following PHP code to your WordPress theme's `functions.php` file or create a custom plugin:
...
### Step 2: Configure Meta Pixel Credentials

1. Access your WordPress admin dashboard
2. Go to `Settings > Meta Pixel`
3. Enter your Meta Pixel ID and Access Token
4. Save the settings

### Step 3: Test Server-Side Tracking

1. Ensure client-side tracking is working first
2. Use your React application to trigger events
3. Check WordPress error logs for server-side tracking logs
4. Verify events appear in Meta Events Manager (they'll be marked as server-side)

## React Application Integration

The React application will automatically use both client-side and server-side tracking. When you call:

```javascript
import { pixelYourSiteService, MetaPixelUserData } from '../services/pixelYourSiteService';

// This will trigger both client-side and server-side tracking
const userData = {
...
pixelYourSiteService.trackPurchase({
  ...
  currency: 'USD',
  contents: [
    { id: 'product-1', quantity: 1, item_price: 99.99 }
  ]
}, userData);
```
...
1. **API calls failing**: Check that your WordPress site allows REST API calls from your React application
2. **Server-side events not appearing**: Check the server logs and ensure the access token has proper permissions
3. **CORS errors**: Ensure CORS headers are properly configured

### Debugging Server-Side Tracking

Server-side tracking events are logged in the WordPress error logs. Check these logs if events aren't being tracked properly.

## Security Considerations

1. Store your Meta Access Token securely in WordPress options
2. The server-side implementation hashes user data for privacy compliance
3. Use HTTPS for all API communications
4. Regularly rotate your Meta Access Token

## Updating and Maintenance

1. Keep the server-side tracking code updated with Meta's latest requirements
2. Monitor server logs for errors
3. Test tracking after WordPress updates
4. Verify tracking continues to work after theme/plugin updates
...
const CartSlide: React.FC<CartSlideProps> = ({ isOpen, onClose }) => {
  const { items, totalItems, totalPrice, updateQuantity, removeItem, addItem } = useCartStore();
  ...
  const animateSlide = useCallback(() => {
    ...
    if (isOpen) {
      setIsVisible(true);
      gsap.set(slideRef.current, { x: '100%', scaleX: 0.95, scaleY: 1 });
      gsap.set(backdropRef.current, { opacity: 0 });
      gsap.to(backdropRef.current, { opacity: 1, duration: 0.3, ease: 'power2.out' });
      gsap.to(slideRef.current, { x: 0, scaleX: 1, duration: 0.4, ease: 'power2.out' });
    } else {
  ...
  return (
    <div className={`fixed inset-0 z-50 overflow-hidden ${isVisible ? "block" : "hidden"}`}>
      {/* Backdrop */}
      <div ref={backdropRef} className="absolute inset-0 bg-black/25" onClick={onClose}></div>

      {/* Slide panel */}
      <div className="absolute inset-y-0 right-0 max-w-full flex">
        <div className="relative w-screen max-w-md">
          <div ref={slideRef} className="h-full flex flex-col bg-white shadow-xl ml-16 w-[calc(100%-4rem)]">

            {/* Header */}
            <div className="flex items-center justify-between px-4 py-4 sm:py-6 border-b border-gray-200">
              <h2 className="text-lg sm:text-xl font-medium text-black font-semibold">Cart</h2>
            ...
            <div className="w-full bg-[#EAE1EF] py-3 overflow-hidden flex-shrink-0">
              <div className="flex whitespace-nowrap animate-marquee px-4 text-xs">{memoizedMessages()}</div>
              <style>{`
                .animate-marquee { display: inline-flex; animation: marquee 7s linear infinite; }
                @keyframes marquee { 0% { transform: translateX(0); } 100% { transform: translateX(-50%); } }
              `}</style>
            </div>

            {/* Cart items - stacked vertically */}
            {/* Cart items - stacked vertically with bigger size */}
<div className="flex-1 flex flex-col min-h-0 p-4 gap-6 overflow-y-auto scrollbar-hidden">
  {items.length === 0 ? (
    ...
    items.map(item => (
      <div key={item.product.id} className="bg-white rounded-lg p-4 flex gap-4 items-center">
        ...
        <div className="flex-1 flex flex-col justify-between h-full">
          <h3 className="text-sm sm:text-base font-medium line-clamp-2">{item.product.name}</h3>
          <div className="flex items-center gap-3 mt-2">
            <span className="text-black font-semibold text-base sm:text-lg">
              ৳{item.product.sale_price && item.product.sale_price !== item.product.regular_price ? item.product.sale_price : item.product.price}
            </span>
            {item.product.sale_price && item.product.sale_price !== item.product.regular_price && (
              <span className="text-gray-500 line-through text-sm sm:text-base">৳{item.product.regular_price}</span>
            )}
          </div>
          <div className="mt-2 flex gap-2 items-center">
            ...
            <div className="pb-4 bg-[#e9e7fd] px-3 py-3 flex-shrink-0">
                    ...
                    <div key={product.id} className="flex-shrink-0 w-[140px] sm:w-[160px]">
                      <div className="bg-white rounded-lg overflow-hidden p-2 flex flex-col h-full">
                        <Link 
                          ...
                          className="block"
                          onClick={() => { closeAllSidebars(); onClose(); }}
                        >
                          <div className="w-full aspect-[3/4]">
                            <img src={product.images[0]?.src || '/placeholder.webp'} alt={product.name} className="w-full h-full object-cover rounded-lg" />
                          </div>
                        ...
                        <button 
                          className="w-full py-1.5 bg-[#D4F871] uppercase rounded-md border-1 text-[10px] sm:text-[11px] border-black flex justify-center items-center gap-1 mt-auto"
                          onClick={(e) => {
                            e.preventDefault();
                            addItem(product, 1);
                          }}

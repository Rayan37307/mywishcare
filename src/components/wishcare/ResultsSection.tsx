...
const ResultsSection: React.FC<ResultsSectionProps> = ({ 
  ...
  React.useEffect(() => {
    const fetchImages = async () => {
      if (wishCare?.resultsImages && wishCare.resultsImages.length > 0) {
  ...
  if (!wishCare?.results && (!wishCare?.resultsImages || wishCare.resultsImages.length === 0)) {
    return null;
  }
  ...
  return (
    <div className="mb-8 p-6 bg-pink-50 rounded-lg">
      ...
      {wishCare.results && (
        <p className="text-gray-700 mb-4">{wishCare.results}</p>
      )}
      {imageUrls.length > 0 && (
        <div className="mt-4">
